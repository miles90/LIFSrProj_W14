...
module Z_AXIS_TRANS_STAGE() {
	...
	rotate([0,180,0]) translate([0,-50,0]) stepper_motor();
...
module z_axis_base() {
	...
	difference() {
		difference() {
			translate([0,-5,-7.5]) cube([base_width,base_length,base_depth],center=true);
		...
		translate([0,-41.5,20]) color("blue") cube([60,20,56], center=true);//cutout for motor seat
		...
		translate([(rod_spacing-10),45,12]) cube([4,12,12],center=true);
		translate([(rod_spacing+40),45,11]) cube([66,11,11],center=true);
		translate([rod_spacing,43,12]) cube([1.5,100,20],center=true);
		translate([(rod_spacing+13),45,11]) rotate([0,90,0]) cylinder(h=50,r=2.5,center=true,$fn=50);
