...
module outer_static_mount() {
	difference() {
		...
		rotate([90,0,0]) translate([mount_position+15,0,68]) cylinder(h=6.1,r=bearing_od/2,center=true,$fn=50); //bearing(2.5,bearing_od/2);
...
module outer_drive_mount() {
	difference() {
		union() {
			translate([62.5,-90,0]) cube([65,50,50],center=true);
			difference() {
				translate([-30,-90,0]) cube([250,50,30],center=true);
				translate([-30,-60,0]) cube([120,60,35],center=true);
				rotate([90,0,0]) translate([-mount_position-80,0,15-10]) smooth_rod(oa_st_rod_l, static_rod_r);
...
module inner_axis() {
	...
	translate([-10,37.5+slide_clearance_w,5]) rotate([0,90,0]) tbs_adj();
	translate([-25,-6,0]) sample_lin_bearing();
	translate([-25,0,0]) sample_arm();
	...
	difference() {
		union() {
			...
			difference() {
				...
				translate([2,-7,20]) rotate([90,0,0]) cylinder(h=25,r=screw_rad,center=true,$fn=50);//screw hole
				translate([2,-7,-10]) rotate([90,0,0]) cylinder(h=25,r=screw_rad,center=true,$fn=50);//screw hole
		...
		translate([5.5,20,0]) cylinder(h=70,r=screw_rad,center=true,$fn=50);	
	}
}
